        ...
        extends PApplet
{
    ...
    */
    ...
    /*private static void loadStrings(String filename, ImageStore imageStore, PApplet screen)
    {
        try
        {
            Scanner in = new Scanner(new File(filename));
            ...
            if (entity instanceof ActivityEntity){
                if (((ActivityEntity)entity).getActionPeriod() > 0) {
                    if (entity instanceof OctoFull) {
                        ((OctoFull) entity).scheduleActions(scheduler, world, imageStore, 0);
                    } else if (entity instanceof OctoNotFull) {
                        ((OctoNotFull) entity).scheduleActions(scheduler, world, imageStore, 0);
                    } else if (entity instanceof Fish) {
                        ((Fish) entity).scheduleActions(scheduler, world, imageStore);
                    } else if (entity instanceof Crab) {
                        ((Crab) entity).scheduleActions(scheduler, world, imageStore, 0);
                    } else if (entity instanceof Quake) {
                        ((Quake) entity).scheduleActions(scheduler, world, imageStore);
                    } else if (entity instanceof Sgrass) {
                        ((Sgrass) entity).scheduleActions(scheduler, world, imageStore);
                    } else if (entity instanceof Atlantis) {
                        ((Atlantis) entity).scheduleActions(scheduler, world, imageStore, ATLANTIS_ANIMATION_REPEAT_COUNT);
                    }
                    else if (entity instanceof Snowflake){
                        ((Snowflake) entity).scheduleActions(scheduler, world, imageStore);
                    }
                }
            }
        }
